...
OLED::OLED(I2C* bus, ssd1306_panel_type_t type,
		uint8_t address) {
	i2c = bus;
	this->type = type;
...
void OLED::command(uint8_t address, uint8_t c) {
	i2c->setAddress(address>>1);
...
void OLED::data(uint8_t address, uint8_t d) {
	i2c->setAddress(address>>1);
