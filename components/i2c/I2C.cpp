...
void I2C::endTransaction() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "i2c_master_cmd_begin: rc=%d %s", errRc, esp_err_to_name(errRc));//GeneralUtils::errorToString(errRc));
	}
...
void I2C::init(uint8_t address, gpio_num_t sdaPin, gpio_num_t sclPin, uint32_t clockSpeed, i2c_port_t portNum, bool pullup) {
	setAddress(address);
	init(sdaPin, sclPin, clockSpeed, portNum, pullup);
}
 ...
 * @return N/A.
 */
void I2C::init(gpio_num_t sdaPin, gpio_num_t sclPin, uint32_t clockSpeed, i2c_port_t portNum, bool pullup) {
	ESP_LOGD(LOG_TAG, ">> I2c::init. sda=%d, scl=%d, clockSpeed=%d, portNum=%d", sdaPin, sclPin, clockSpeed, portNum);
...
void I2C::read(uint8_t* bytes, size_t length, bool ack) {
	if (debug) {
		ESP_LOGD(LOG_TAG, "read(addr=0x%x, size=%d, ack=%d)", getAddress(), length, ack);
	}
	if (!m_directionKnown) {
...
void I2C::read(uint8_t* byte, bool ack) {
	if (debug) {
		ESP_LOGD(LOG_TAG, "addr=0x%x, read(size=1, ack=%d)", getAddress(), ack);
	}
	if (!m_directionKnown) {
		m_directionKnown = true;
		esp_err_t errRc = ::i2c_master_write_byte(m_cmd, (m_address << 1) | I2C_MASTER_READ, !ack);
...
void I2C::write(uint8_t byte, bool ack) {
	if (debug) {
		ESP_LOGD(LOG_TAG, "write(addr=0x%x, val=0x%.2x, ack=%d)", getAddress(), byte, ack);
	}
	if (!m_directionKnown) {
		m_directionKnown = true;
...
void I2C::write(uint8_t *bytes, size_t length, bool ack) {
	if (debug) {
		ESP_LOGD(LOG_TAG, "write(addr=0x%x, length=%d, ack=%d)", getAddress(), length, ack);
	}
	if (!m_directionKnown) {
		m_directionKnown = true;
		esp_err_t errRc = ::i2c_master_write_byte(m_cmd, (m_address << 1) | I2C_MASTER_WRITE, !ack);
